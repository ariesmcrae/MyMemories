 ...
 * 
 */
public class MyMemoriesResolver {

	private ContentResolver cr;

	private Uri storyURI = MyMemoriesSchema.Story.CONTENT_URI;
	private Uri tagsURI = MyMemoriesSchema.Tags.CONTENT_URI;
	 ...
	 */
	public MyMemoriesResolver(Activity activity) {
	...
	public ContentProviderResult[] applyBatch(
		...
		return cr.applyBatch(MyMemoriesSchema.AUTHORITY, operations);
	}
	...
	public Uri insert(final StoryData storyObject) throws RemoteException {
		...
		tempCV.remove(MyMemoriesSchema.Story.Cols.ID);
	...
	public Uri insert(final TagsData tagsObject) throws RemoteException {
		...
		tempCV.remove(MyMemoriesSchema.Tags.Cols.ID);
	...
	public StoryData getStoryDataViaRowID(final long rowID)
		...
		ArrayList<StoryData> results = queryStoryData(null,
				MyMemoriesSchema.Story.Cols.ID + "= ?", selectionArgs, null);
		if (results.size() > 0) {
	...
	public TagsData getTagsDataViaRowID(final long rowID)
		...
		ArrayList<TagsData> results = queryTagsData(null,
				MyMemoriesSchema.Tags.Cols.ID + "= ?", selectionArgs, null);
		if (results.size() > 0) {
			return results.get(0);
		} else {
			return null;
	...
	public int deleteAllStoryWithRowID(long rowID) throws RemoteException {
		...
		return deleteStoryData(MyMemoriesSchema.Story.Cols.ID + " = ? ", args);
	}
	...
	public int deleteAllTagsWithRowID(long rowID) throws RemoteException {
		String[] args = { String.valueOf(rowID) };
		return deleteTagsData(MyMemoriesSchema.Tags.Cols.ID + " = ? ", args);
	}
