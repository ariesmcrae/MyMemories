/*
The MyMemoriesContentProvider source code (henceforth referred to as "MyMemoriesContentProvider") is
...
2014, all rights reserved.  Since MyMemoriesContentProvider is open-source, freely
...
statement along with any code built using MyMemoriesContentProvider that you release. No
copyright statement needs to be provided if you just ship binary
executables of your software products.

You can use MyMemoriesContentProvider software in commercial and/or binary software
...
misappropriate the MyMemoriesContentProvider code, such as copyrighting it yourself or
claiming authorship of the MyMemoriesContentProvider software code, in a way that will
prevent the software from being distributed freely using an open-source
development model. You needn't inform anyone that you're using MyMemoriesContentProvider
software in your software, though we encourage you to let us know so we
can promote your project in our success stories.

MyMemoriesContentProvider is provided as is with no warranties of any kind, including
...
MyMemoriesContentProvider is provided with no support and without any obligation on the
...
products or services derived from the MyMemoriesContentProvider source, nor does it
grant permission for the name Vanderbilt University or
University of Maryland to appear in their names.
 */

package com.ariesmcrae.mymemoriescontentprovider.orm;

import java.util.ArrayList;

import com.ariesmcrae.mymemoriescontentprovider.provider.MyMemoriesSchema;
...
public class StoryCreator {
	...
	public static StoryData getStoryDataFromCursor(Cursor cursor) {
		...
		long rowID = cursor.getLong(cursor
				.getColumnIndex(MyMemoriesSchema.Story.Cols.ID));
		long loginId = cursor.getLong(cursor
				.getColumnIndex(MyMemoriesSchema.Story.Cols.LOGIN_ID));
		long storyId = cursor.getLong(cursor
				.getColumnIndex(MyMemoriesSchema.Story.Cols.STORY_ID));
		String title = cursor.getString(cursor
				.getColumnIndex(MyMemoriesSchema.Story.Cols.TITLE));
		String body = cursor.getString(cursor
				.getColumnIndex(MyMemoriesSchema.Story.Cols.BODY));
		String audioLink = cursor.getString(cursor
				.getColumnIndex(MyMemoriesSchema.Story.Cols.AUDIO_LINK));
		String videoLink = cursor.getString(cursor
				.getColumnIndex(MyMemoriesSchema.Story.Cols.VIDEO_LINK));
		String imageName = cursor.getString(cursor
				.getColumnIndex(MyMemoriesSchema.Story.Cols.IMAGE_NAME));
		String imageMetaData = cursor.getString(cursor
				.getColumnIndex(MyMemoriesSchema.Story.Cols.IMAGE_LINK));
		String tags = cursor.getString(cursor
				.getColumnIndex(MyMemoriesSchema.Story.Cols.TAGS));
		long creationTime = cursor.getLong(cursor
				.getColumnIndex(MyMemoriesSchema.Story.Cols.CREATION_TIME));
		long storyTime = cursor.getLong(cursor
				.getColumnIndex(MyMemoriesSchema.Story.Cols.STORY_TIME));
		double latitude = cursor.getDouble(cursor
				.getColumnIndex(MyMemoriesSchema.Story.Cols.LATITUDE));
		double longitude = cursor.getDouble(cursor
				.getColumnIndex(MyMemoriesSchema.Story.Cols.LONGITUDE));
